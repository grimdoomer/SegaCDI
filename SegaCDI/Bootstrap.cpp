...
bool CBootstrap::LoadBootstrap(char *pbBuffer, int dwBufferSize)
{
...
void CBootstrap::PatchBootstrapForRegion(int dwRegion)
{
...
bool CBootstrap::CopyBootstrapBuffer(char *pbBuffer, int dwBufferSize)
{
...
bool CBootstrap::Extract3rdPartyBootLogo(const char *psOutputFolder)
{
	// Check if a 3rd party boot logo exists in bootstrap 1.
	if (*(short*)&this->m_sBootstrap.bBootstrap1[32] != ByteFlip16(MR_IMAGE_MAGIC))
	{
		// Print error and return false.
		printf("ERROR: bootstrap does not contain a 3rd-party boot logo!\n");
		return false;
	}
...
bool CBootstrap::Inject3rdPartyBootLogo(const char *pbBuffer, int dwBufferSize)
{
